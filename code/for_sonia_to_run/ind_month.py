...
pred_inputs_=pd.read_excel(data_file,sheet_name=0,header=0,index_col=False,keep_default_na=True)
##########################################################################################################################

#change directory to put the iso info
wan_mon=[1,2,3,12,6,7,8]
for m in wan_mon:
    #data preparation
    patth=os.path.join(r"C:\Users\usuari\Desktop\10_12_2020\iso_models\12_12_2020\ind_month", "month_"+str(m))
    preped_dataset=data_preparation.preprocess(meteo_input_type="monthly",direc=patth) #your working folder
    #meteo_input_type="daily_remove_outliers"
    preped_dataset.fit(rain,temp,hum,iso_18,iso_2h,iso_3h)
    preped_dataset.direc=patth
    ##########################################################################################################################
    # colgas meeo model:
    ...
    iso_meteo_model.iso_predic(preped_dataset,run_iso_whole_year=False,iso_model_month_list=[m]) #run_iso_whole_year=False,iso_model_month_list=[1,2,3,12]
    ##########################################################################################################################
    #iso model
    ...
    ev_class.predict(iso_meteo_model,pred_inputs,direc=os.path.join(r"C:\Users\usuari\Desktop\10_12_2020\iso_models\12_12_2020\predictions\preds_indv_obs", "preds_obs_month_" + str(m)))
    ...
    ev_class_.predict(iso_meteo_model,pred_inputs_,direc=os.path.join(r"C:\Users\usuari\Desktop\10_12_2020\iso_models\12_12_2020\predictions\preds__indv_map", "preds_map_month_"+str(m)))
    #cambia esta linea si quieres correr para hacer el mapa grande de prediction:
